        ...
        extends BaseJdbcClient
{
    ...
    private static final LocalDate MIN_DATE = LocalDate.parse("0000-01-01");
    ...
    private static void checkDateValue(long epochDay)
    {
        if (epochDay < MIN_DATE.toEpochDay() || epochDay > MAX_DATE.toEpochDay()) {
            throw new TrinoException(INVALID_ARGUMENTS, format("Date must be between %s and %s in StarRocks: %s", MIN_DATE, MAX_DATE, LocalDate.ofEpochDay(epochDay)));
        }
    }
